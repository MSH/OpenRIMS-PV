...
export class OutstandingVisitComponent extends BaseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    this.eventService.removeAll(OutstandingVisitComponent.name);
  }  
  ...
  loadGrid(): void {
    ...
    self.appointmentService.getOutstandingVisitReport(self.viewModel.mainGrid.customFilterModel(self.viewModelForm.value))
        .pipe(takeUntil(self._unsubscribeAll))
        .pipe(finalize(() => self.setBusy(false)))
        .subscribe(result => {
            self.viewModel.mainGrid.updateAdvance(result);
        }, error => {
            self.handleError(error, "Error fetching outstanding visit report");
        });
  }  
...
class ViewModel {
  mainGrid: GridModel<GridRecordModel> =
      new GridModel<GridRecordModel>
          (['first-name', 'last-name', 'facility', 'appointment-date']);
