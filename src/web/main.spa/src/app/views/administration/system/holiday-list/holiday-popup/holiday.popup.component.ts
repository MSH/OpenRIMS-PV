...
export class HolidayPopupComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    self.itemForm = this.formBuilder.group({
      holidayDate: [this.data.payload.holidayDate, Validators.required],
      description: [this.data.payload.description || '', [Validators.required, Validators.maxLength(100)]],
    })
  }
  ...
  submit() {
    ...
    self.holidayService.saveHoliday(self.data.holidayId, self.itemForm.value)
      .pipe(finalize(() => self.setBusy(false)))
      .subscribe(result => {
        self.notify("Holiday saved successfully", "Public Holidays");
