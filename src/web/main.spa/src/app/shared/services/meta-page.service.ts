...
export class MetaPageService extends BaseService {
    ...
    return this.getMetaPages(filter)
      .pipe( 
        expand(response => {
          ...
          return next ? this.GetByAddress<MetaPageDetailWrapperModel>(next.href, 'application/vnd.main.detail.v1+json') : EMPTY;
        }),
        map(response => {
          let typedResponse = response as MetaPageDetailWrapperModel;
  ...
  getMetaPages(filterModel: any): any {
    ...
    return this.Get<MetaPageDetailWrapperModel>('/metapages', 'application/vnd.main.detail.v1+json', parameters);
  }
  ...
  getMetaPage(id: number, header: 'detail' | 'expanded' ): any {
    ...
    return this.Get<MetaPageExpandedModel>('/metapages', `application/vnd.main.${header}.v1+json`, parameters);
  }
