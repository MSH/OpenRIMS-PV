...
export class RiskFactorService extends BaseService {
      ...
      return this.getRiskFactors(filter)
        .pipe( 
          expand(response => {
            ...
            return next ? this.GetByAddress<RiskFactorDetailWrapperModel>(next.href, 'application/vnd.main.detail.v1+json') : EMPTY;
          }),
          map(response => {
            let typedResponse = response as RiskFactorDetailWrapperModel;
    ...
    getRiskFactors(filterModel: any): any {
      ...
      return this.Get<RiskFactorDetailWrapperModel>('', 'application/vnd.main.detail.v1+json', parameters);
    }    
}
