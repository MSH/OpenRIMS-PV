...
export class EncounterTypeService extends BaseService {
    ...
    return this.getEncounterTypes(filter)
      .pipe( 
        expand(response => {
          ...
          return next ? this.GetByAddress<EncounterTypeIdentifierWrapperModel>(next.href, 'application/vnd.main.identifier.v1+json') : EMPTY;
        }),
        map(response => {
          let typedResponse = response as EncounterTypeIdentifierWrapperModel;
  ...
  getEncounterTypes(filterModel: any): any {
    ...
    return this.Get<EncounterTypeIdentifierWrapperModel>('/encountertypes', 'application/vnd.main.identifier.v1+json', parameters);
}
  ...
  getEncounterTypesByDetail(filterModel: any): any {
      ...
      return this.Get<EncounterTypeDetailWrapperModel>('/encountertypes', 'application/vnd.main.detail.v1+json', parameters);
  }
