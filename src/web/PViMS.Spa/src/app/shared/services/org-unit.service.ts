...
export class OrgUnitService extends BaseService {
      ...
      return this.getOrgUnits(filter)
        .pipe( 
          expand(response => {
            ...
            return next ? this.GetByAddress<OrgUnitIdentifierWrapperModel>(next.href, 'application/vnd.pvims.identifier.v1+json') : EMPTY;
          }),
          map(response => {
            let typedResponse = response as OrgUnitIdentifierWrapperModel;
    ...
    getOrgUnits(filterModel: any): any {
      ...
      return this.Get<OrgUnitIdentifierWrapperModel>('/orgunits', 'application/vnd.pvims.identifier.v1+json', parameters);
    }
}
