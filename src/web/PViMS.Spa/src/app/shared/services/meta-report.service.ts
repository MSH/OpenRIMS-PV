...
export class MetaReportService extends BaseService {
    ...
    return this.getMetaReportsByDetail(filter)
      .pipe( 
        expand(response => {
          ...
          return next ? this.GetByAddress<MetaReportDetailWrapperModel>(next.href, 'application/vnd.pvims.detail.v1+json') : EMPTY;
        }),
        map(response => {
          let typedResponse = response as MetaReportDetailWrapperModel;
  ...
  getMetaReportsByDetail(filterModel: any): any {
    ...
    return this.Get<MetaReportDetailWrapperModel>('/metareports', 'application/vnd.pvims.detail.v1+json', parameters);
  }
  ...
  getMetaReportByDetail(id: number): any {
    ...
    return this.Get<MetaReportExpandedModel>('/metareports', `application/vnd.pvims.detail.v1+json`, parameters);
  }
  ...
  getMetaReportByExpanded(id: number): any {
    ...
    return this.Get<MetaReportExpandedModel>('/metareports', `application/vnd.pvims.expanded.v1+json`, parameters);
  }
