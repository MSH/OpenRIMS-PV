...
using OpenRIMS.PV.Main.API.Application.Commands.ConceptAggregate;

namespace OpenRIMS.PV.Main.API.Application.Validations
{
    public class AddConceptCommandValidator : AbstractValidator<AddConceptCommand>
    {
        public AddConceptCommandValidator(ILogger<AddConceptCommandValidator> logger)
        {
            RuleFor(command => command.ConceptName)
                .NotEmpty()
                .Length(1, 1000)
                .Matches(@"[-a-zA-Z0-9 .,()%/]")
                .WithMessage("Concept name contains invalid characters (Enter A-Z, a-z, 0-9, space, hyphen, period, comma, brackets, percentage, forward slash)");
