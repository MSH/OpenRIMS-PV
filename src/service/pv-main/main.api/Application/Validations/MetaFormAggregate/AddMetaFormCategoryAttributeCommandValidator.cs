...
{
    public class AddMetaFormCategoryAttributeCommandValidator : AbstractValidator<AddMetaFormCategoryAttributeCommand>
    {
        public AddMetaFormCategoryAttributeCommandValidator(ILogger<AddMetaFormCategoryAttributeCommandValidator> logger)
        {
            RuleFor(command => command.AttributeName)
                .NotEmpty()
                .Matches(@"[-a-zA-Z0-9 .,()']")
                .WithMessage("Attribute name contains invalid characters (Enter A-Z, a-z, 0-9, hyphen, space, period, comma, parentheses, apostrophe)");
